...
unshift(@paths, qw(/usr/local/nrpe /usr/sbin /usr/local/bin /sbin /bin /usr/sbin));
...
sub check {
    ...
    unless (%targets) {
        # No controllers not necessarily an fault/warning
        # $this->warning;
        $this->ok;
...
# check to insure that the effective UID is root before continuing
if ($> != 0) {
    exit 0;
}
...
if ($message) {
    if ($status == $ERRORS{OK}) {
        print "raid_status=OK: ";
    } elsif ($status == $ERRORS{WARNING}) {
        print "raid_status=WARNING: ";
    } elsif ($status == $ERRORS{CRITICAL}) {
        print "raid_status=CRITICAL: ";
    } else {
        print "raid_status=UNKNOWN: ";
    }
    print "$message\n";
} else {
    $status = $ERRORS{UNKNOWN};
    print "raid_status=No RAID configuration found (tried: ", join(', ', @plugins), ")\n";
}
exit $status;
